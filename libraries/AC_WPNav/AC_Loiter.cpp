...
void AC_Loiter::init_target(const Vector3f& position)
{
    ...
    _pos_control.set_max_speed_accel_xy(LOITER_VEL_CORRECTION_MAX, 0.5f * _accel_cmss);
    _pos_control.set_max_accel_correction_max_xy_cmss(_accel_cmss);
    ...
    // set target position
    _pos_control.set_pos_target_xy(position.x, position.y);
}
