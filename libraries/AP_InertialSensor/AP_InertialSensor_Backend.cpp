                                                            ...
                                                            uint64_t sample_us)
{
...

    {
        ...
        // apply the dynamic notch filter
        if (_gyro_dynamic_notch_enabled()) {
            _imu._gyro_filtered[instance] = _imu._gyro_dynamic_notch_filter[instance].apply(_imu._gyro_filtered[instance]);
        }
        ...
        if (_imu._gyro_filtered[instance].is_nan() || _imu._gyro_filtered[instance].is_inf()) {
            ...
            _imu._gyro_dynamic_notch_filter[instance].reset();
        }
...
{    
    ...
    if (_last_dynamic_notch_center_freq_hz[instance] != _gyro_dynamic_notch_center_freq_hz()) {
        _imu._gyro_dynamic_notch_filter[instance].update(_gyro_dynamic_notch_center_freq_hz());
        _last_dynamic_notch_center_freq_hz[instance] = _gyro_dynamic_notch_center_freq_hz();
    } 
    // possily update the dynamic notch filter parameters
    if (_last_dynamic_notch_bandwidth_hz[instance] != _gyro_dynamic_notch_bandwidth_hz() ||
        !is_equal(_last_dynamic_notch_attenuation_dB[instance], _gyro_dynamic_notch_attenuation_dB())) {
        _imu._gyro_dynamic_notch_filter[instance].init(_gyro_raw_sample_rate(instance), _gyro_dynamic_notch_center_freq_hz(), _gyro_dynamic_notch_bandwidth_hz(), _gyro_dynamic_notch_attenuation_dB());
        _last_dynamic_notch_center_freq_hz[instance] = _gyro_dynamic_notch_center_freq_hz();
