...
void spline_curve::set_speed_accel(float speed_xy_cms, float speed_up_cms, float speed_down_cms,
...
void spline_curve::set_origin_and_destination(const Vector3f &origin, const Vector3f &destination, const Vector3f &origin_vel, const Vector3f &destination_vel)
{
...
void spline_curve::calc_dt_speed_max(float time, float distance_delta, float &spline_dt, Vector3f &target_pos, Vector3f &spline_vel_unit, float &speed_xy_max)
{
    ...
    float speed_max = kinematic_limit(spline_vel_unit, _speed_xy_cms, _speed_up_cms, _speed_down_cms);
    float accel_norm_max = 0.5f * kinematic_limit(spline_accel_norm, _accel_xy_cmss, _accel_z_cmss, _accel_z_cmss);
    ...
    float accel_max = 0.5f * kinematic_limit(spline_vel_unit, _accel_xy_cmss, _accel_z_cmss, _accel_z_cmss);
