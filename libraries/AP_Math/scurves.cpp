...
void scurves::set_speed(float speed_xy, float speed_up, float speed_down)
{
    // segment accelerations can not be changed after segment creation.
    float path_speed_max = kinematic_limit(_delta_unit, speed_xy, speed_up, fabsf(speed_down));

    if (is_equal(vel_max, path_speed_max)) {
        // New speed is equal to current speed maximum
        return;
    }

    if (is_zero(vel_max) || is_zero(path_speed_max)) {
        // New or original speeds are set to zero
        return;
    }
    set_speed_max(path_speed_max);
    ...
    if ( (_t <= segment[SEG_ACCEL_MAX].end_time) && is_positive(segment[SEG_ACCEL_MAX].end_time - segment[SEG_ACCEL_MAX-1].end_time) && (vel_max < segment[SEG_ACCEL_END].end_vel) && is_positive(segment[SEG_ACCEL_MAX].end_accel) ) {
        ...
        float Jm, t2, t4, t6;
        calculate_path(Vstart, otj, jerk_max, accel_max, MAX(Vmin, vel_max), Pend / 2.0, Jm, t2, t4, t6);
        ...
        calculate_path(0.0f, otj, jerk_max, accel_max, MAX(Vmin, vel_max), Pend / 2.0, Jm, t2, t4, t6);
    ...
    if (!is_equal(vel_max, segment[SEG_ACCEL_END].end_vel)) {
        ...
        if ((vel_max < segment[SEG_ACCEL_END].end_vel) && (otj*12.0f < L/segment[SEG_ACCEL_END].end_vel)) {
            ...
            calculate_path(vel_max, otj, jerk_max, accel_max, segment[SEG_ACCEL_END].end_vel, L / 2.0, Jm, t6, t4, t2);
            Jm = -Jm;

        } else if ((vel_max > segment[SEG_ACCEL_END].end_vel) && (L/(otj*12.0f) > segment[SEG_ACCEL_END].end_vel)) {
            float Vm = MIN(vel_max, L/(otj*12.0f));
            calculate_path(segment[SEG_ACCEL_END].end_vel, otj, jerk_max, accel_max, Vm, L / 2.0, Jm, t2, t4, t6);
        }
    ...
    if (Vend < segment[SEG_CHANGE_END].end_vel) {
        float Jm, t2, t4, t6;
        calculate_path(Vend, otj, jerk_max, accel_max, segment[SEG_CONST].end_vel, Pend - segment[SEG_CONST].end_pos, Jm, t2, t4, t6);
        add_segments_jerk(seg, otj, -Jm, t6);
        add_segment_const_jerk(seg, t4, 0.0);
        add_segments_jerk(seg, otj, Jm, t2);
    } else {
                   ...
                   float accel_xy, float accel_z)
{
    ...
    Vector3f track = destination - origin;
    float path_length = track.length();
    if (is_zero(path_length)) {
        // avoid possible divide by zero
        _delta_unit.zero();
    } else {
        _delta_unit = track.normalized();
...
float scurves::set_origin_speed_max(float speed)
{
    ...
    float Jm, t2, t4, t6;
    calculate_path(speed, otj, jerk_max, accel_max, Vm, L / 2.0, Jm, t2, t4, t6);
    ...
    calculate_path(0.0f, otj, jerk_max, accel_max, Vm, L - segment[SEG_CONST].end_pos, Jm, t2, t4, t6);
...
void scurves::move_to_pos_vel_accel(float dt, Vector3f &pos, Vector3f &vel, Vector3f &accel)
{
    advance_time(dt);
    move_from_time_pos_vel_accel(_t, pos, vel, accel);
    pos -= _delta_unit * segment[num_segs].end_pos;
}
...
void scurves::add_segments(float L)
{
    ...
    float Jm, t2, t4, t6;
    calculate_path(0.0f, otj, jerk_max, accel_max, vel_max, L / 2.0, Jm, t2, t4, t6);
...
void scurves::calculate_path(float V0, float tj, float Jm, float Am, float Vm, float L, float &Jm_out, float &t2_out, float &t4_out, float &t6_out) const
{
    if (!is_positive(tj) || !is_positive(Jm) || !is_positive(Am) || !is_positive(Vm) || !is_positive(L) || V0 >= Vm) {
...
{
    add_segment_increasing_jerk(seg_pnt, tj, Jm);
    add_segment_const_jerk(seg_pnt, Tcj, Jm);
    add_segment_decrease_jerk(seg_pnt, tj, Jm);
}
...
void scurves::add_segment_increasing_jerk(uint16_t &seg_pnt, float tj, float Jm)
{
    float Beta = M_PI / tj;
    float Alpha = Jm / 2.0;
    float AT = Alpha * tj;
...
void scurves::add_segment_decrease_jerk(uint16_t &seg_pnt, float tj, float Jm)
{
    float Beta = M_PI / tj;
    float Alpha = Jm / 2.0;
    float AT = Alpha * tj;
                                   ...
                                   float accel_xy, float accel_z)
{
    ...
    float path_speed_max = kinematic_limit(direction, speed_xy, speed_up, speed_down);
    float path_accel_max = kinematic_limit(direction, accel_xy, accel_z, accel_z);

    set_speed_max(path_speed_max);
    set_accel_max(path_accel_max);
}
