...
float AC_P_1D::update_all(float &target, float measurement, bool &limit_min, bool &limit_max)
{
    ...
    asymetric_limit(error, _lim_err_neg, _lim_err_pos, limit_min, limit_max );
...
/// set limits based on separate min and max
bool AC_P_1D::asymetric_limit(float &input, float min, float max, bool &limit_min, bool &limit_max )
{
    if (input < min) {
        input = min;
        limit_min = true;
