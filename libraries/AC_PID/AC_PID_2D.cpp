...
Vector2f AC_PID_2D::update_all(float target_x, float target_y, const Vector2f &measurement, bool limit)
{
    // don't process inf or NaN
    if (!isfinite(target_x) || isnan(target_x) ||
        !isfinite(target_y) || isnan(target_y) ||
        !isfinite(measurement.x) || isnan(measurement.x) ||
        !isfinite(measurement.y) || isnan(measurement.y)) {
        return Vector2f();
    }
...
Vector2f AC_PID_2D::update_all(float target_x, float target_y, const Vector3f &measurement, bool limit)
{
