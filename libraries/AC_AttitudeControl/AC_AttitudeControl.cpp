...
void AC_AttitudeControl::input_thrust_vector_rate_heading(Vector3f thrust_vector, float heading_rate_cds)
{
    ...
    const float thrust_vector_angle = acosf(constrain_float(Vector3f(0.0f,0.0f,-1.0f) * thrust_vector, -1.0f, 1.0f));
    ...
    thrust_vector_rotation_angles(thrust_vec_quat, _attitude_target, thrust_vec_correction_quat, rotation, returned_thrust_vector_angle, thrust_vector_diff_angle);
...
// Command an euler roll and pitch angle and an euler yaw rate with angular velocity feedforward and smoothing
void AC_AttitudeControl::input_thrust_vector_heading(Vector3f thrust_vector, float heading_angle_cd, float heading_rate_cds)
{
    // Convert from centidegrees on public interface to radians
    const float heading_rate = radians(heading_rate_cds * 0.01f);
    const float heading_angle = radians(heading_angle_cd * 0.01f);
    ...
    const Quaternion desired_attitude_quat = thrust_vec_quat*yaw_quat;
    ...
    if (_rate_bf_ff_enabled) {
        ...
        Quaternion thrust_vec_correction_quat;
        float returned_thrust_vector_angle;
        thrust_vector_rotation_angles(desired_attitude_quat, _attitude_target, thrust_vec_correction_quat, rotation, returned_thrust_vector_angle, thrust_vector_diff_angle);
