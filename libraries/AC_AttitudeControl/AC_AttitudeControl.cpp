...
void AC_AttitudeControl::input_thrust_vector_heading_rate_heading(Vector3f att_to_thrust_vec, float euler_yaw_angle_cd, float euler_yaw_rate_cds)
{
    // Convert from centidegrees on public interface to radians
    float euler_yaw_angle = radians(euler_yaw_angle_cd * 0.01f);
    float euler_yaw_rate = radians(euler_yaw_rate_cds * 0.01f);

    if (is_zero(att_to_thrust_vec.length())) {
        att_to_thrust_vec = Vector3f(0.0f, 0.0f, 1.0f);
    } else {
        att_to_thrust_vec.normalize();
    }
    ...
    float thrust_error_angle = acosf(constrain_float(att_from_thrust_vec * att_to_thrust_vec, -1.0f, 1.0f));
    ...
    thrust_vec_correction_quat = _attitude_target_quat.inverse() * thrust_vec_correction_quat * _attitude_target_quat;

    // calculate the angle error in x and y.
    Vector3f rotation;
    Vector3f attitude_error_angle;
    thrust_vec_correction_quat.to_axis_angle(attitude_error_angle);
    attitude_error_angle.z = wrap_PI(euler_yaw_angle - _attitude_target_euler_angle.z);
...
//    if (_rate_bf_ff_enabled) {
        ...
        // angular velocity that will cause the euler angle to smoothly stop at the input angle with limited deceleration
        // and an exponential decay specified by _input_tc at the end.
        _attitude_target_ang_vel.x = input_shaping_angle(wrap_PI(attitude_error_angle.x), 0.0f, get_accel_roll_max_radss(), _attitude_target_ang_vel.x, _dt);
        _attitude_target_ang_vel.y = input_shaping_angle(wrap_PI(attitude_error_angle.y), 0.0f, get_accel_pitch_max_radss(), _attitude_target_ang_vel.y, _dt);

        _attitude_correction_euler_rate.x = 0.0f;
        _attitude_correction_euler_rate.y = 0.0f;
        _attitude_correction_euler_rate.z = input_shaping_angle(wrap_PI(euler_yaw_angle - _attitude_target_euler_angle.z), _input_tc, get_accel_yaw_max_radss(), _attitude_target_ang_vel.z, _dt);
