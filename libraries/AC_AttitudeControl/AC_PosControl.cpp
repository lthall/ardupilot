...
{
    const float sin_yaw = sinf(_attitude_control.get_att_target_euler().z);
    const float cos_yaw = cosf(_attitude_control.get_att_target_euler().z);

    // rotate accelerations into body forward-right frame
    // todo: this should probably be based on the desired heading not the current heading
    const float accel_forward = accel_x_cmss * cos_yaw + accel_y_cmss * sin_yaw;
    const float accel_right = -accel_x_cmss * sin_yaw + accel_y_cmss * cos_yaw;
