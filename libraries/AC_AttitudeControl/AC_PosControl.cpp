...
void AC_PosControl::set_max_speed_accel_xy(float speed_cms, float accel_cmss, float accel_limit_cmss)
{
    ...
    if ( is_positive(accel_limit_cmss)) {
        ...
        accel_cmss = MIN(_accel_max_xy_cmss, 0.5f * accel_limit_cmss);
    }

    _p_pos_xy.set_limits(0.0f, _vel_max_xy_cms, accel_cmss, 0.0f);
...
void AC_PosControl::update_xy_controller()
{
    ...
    if (!is_zero(_accel_limit_xy_cmss)) {
        _limit.accel_xy = accel_target.limit_length(_accel_limit_xy_cmss);
    }
