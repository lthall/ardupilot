...
void Mode::_TakeOff::do_pilot_takeoff(float& pilot_climb_rate_cm)
{
    ...
    // stop take off early and return if negative climb rate is commanded or we are within 1% of our take off altitude
    if (is_negative(pilot_climb_rate_cm) ||
        (take_off_alt - take_off_start_alt) * 0.99f < copter.pos_control->get_pos_target_z_cm() - take_off_start_alt) {
        stop();
    }
}
