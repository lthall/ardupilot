...
void ModeAuto::do_land(const AP_Mission::Mission_Command& cmd)
{
    ...
    if (cmd.content.location.lat != 0 || cmd.content.location.lng != 0) {
        ...
        const Location dest_loc = terrain_adjusted_location(cmd);
...
void ModeAuto::do_loiter_unlimited(const AP_Mission::Mission_Command& cmd)
{
    ...
    Location dest_loc(cmd.content.location);

    // use current location if not provided
    if (dest_loc.lat == 0 && dest_loc.lng == 0) {
        // To-Do: make this simpler
        Vector3f temp_pos;
        copter.wp_nav->get_wp_stopping_point_xy(temp_pos);
        const Location temp_loc(temp_pos);
        dest_loc.lat = temp_loc.lat;
        dest_loc.lng = temp_loc.lng;
    }
    ...
    if (dest_loc.alt == 0) {
        ...
        int32_t curr_alt;
        if (copter.current_loc.get_alt_cm(dest_loc.get_alt_frame(),curr_alt)) {
            dest_loc.set_alt_cm(curr_alt, dest_loc.get_alt_frame());
        } else {
            // default to current altitude as alt-above-home
            dest_loc.set_alt_cm(copter.current_loc.alt,
...
bool ModeAuto::verify_payload_place()
{
    ...
    case PayloadPlaceStateType_Released: {
        ...
        FALLTHROUGH;
    case PayloadPlaceStateType_Ascending_Start: {
        Location dest_loc = inertial_nav.get_position();
        dest_loc.alt = nav_payload_place.descend_start_altitude;
        wp_nav->set_wp_destination_loc(dest_loc);
        nav_payload_place.state = PayloadPlaceStateType_Ascending;
        }
        FALLTHROUGH;
