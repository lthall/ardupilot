...
void ModeFollow::run()
{
    ...
    if (g2.follow.update_target_pos_and_vel()) {
        g2.follow.get_target_dist_and_vel_ned(dist_vec_offs, vel_of_target);
        ...
        // create horizontal desired velocity vector (required for avoid calculations)
