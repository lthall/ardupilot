...
bool ModeGuided::set_destination(const Vector3f& destination, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw, bool terrain_alt)
{
    ...
    // log target
    copter.Log_Write_GuidedTarget(guided_mode, guided_pos_target_cm, guided_vel_target_cms);
...
bool ModeGuided::set_destination(const Location& dest_loc, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw)
{
    ...
    // log target
    copter.Log_Write_GuidedTarget(guided_mode, Vector3f(dest_loc.lat, dest_loc.lng, dest_loc.alt),Vector3f(1.0f, 2.0f, 3.0f));
...
{
    return ((copter.g2.guided_options.get() & uint32_t(Options::DoNotStabilizePositionXY)) == 1);
}

// returns true if GUIDED_OPTIONS param suggests SET_ATTITUDE_TARGET's "thrust" field should be interpreted as thrust instead of climb rate
bool ModeGuided:: stop_stabilize_vel_xy() const
{
    return ((copter.g2.guided_options.get() & uint32_t(Options::DoNotStabilizeVelocityXY)) == 1);
}
