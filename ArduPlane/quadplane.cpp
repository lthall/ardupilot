...
void QuadPlane::control_loiter()
{
    ...
    if (plane.control_mode == &plane.mode_qland) {
        ...
        pos_control->set_alt_target_from_climb_rate(get_pilot_desired_climb_rate_cms(), false);
    }
    run_z_controller();
}
...
void QuadPlane::vtol_position_controller(void)
{
    ...
    case QPOS_POSITION1: {
        ...
        // don't allow down pitch during this phase of landing. If we
        // get too slow then we switch to QPOS_POSITION2
        const float pitch_limit_cd = 0;
        if (plane.nav_pitch_cd < pitch_limit_cd) {
            plane.nav_pitch_cd = pitch_limit_cd;
    ...
    case QPOS_LAND_FINAL: {
        ...
        vtol_position_xy(pos, accel_xy_limit_cmss);
...
void QuadPlane::setup_target_position(void)
{
    ...
    pos_control->set_max_speed_accel_xy(max_speed, wp_nav->get_wp_acceleration());
}
...
void QuadPlane::takeoff_controller(void)
{
    ...
    */
    ...
    vtol_position_xy(pos, wp_nav->get_wp_acceleration());
    ...
    pos_control->set_alt_target_from_climb_rate(wp_nav->get_default_speed_up(), false);
    run_z_controller();
}
...
void QuadPlane::waypoint_controller(void)
{
    ...
    */
    ...
    pos_control->set_alt_target_from_climb_rate(assist_climb_rate_cms(), false);
    run_z_controller();
}
...
bool QuadPlane::in_vtol_land_approach(void) const
{
    ...
    if (in_vtol_auto()) {
        ...
        if ((options & QuadPlane::OPTION_MISSION_LAND_FW_APPROACH) &&
            ((plane.vtol_approach_s.approach_stage == Plane::Landing_ApproachStage::APPROACH_LINE) ||
